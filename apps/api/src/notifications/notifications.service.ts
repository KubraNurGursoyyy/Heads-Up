...
export class NotificationsService {
  ...
  async send(
    ...
    const existingEvent = await this.p.notification.findUnique({
      where: { watchId_eventKey: { watchId, eventKey } },
    });
    if (existingEvent) {
    ...
    const existingArticle = await this.p.notification.findUnique({
      where: { watchId_articleId: { watchId, articleId } },
    });
    if (existingArticle)
    ...
    const notification = await this.p.notification.create({
      data: { userId, watchId, articleId, eventKey, url, title, body },
    });
  ...
  private async deliver(notification: Notification) {
    const devices = await this.p.device.findMany({
      where: { userId: notification.userId, enabled: true },
    });
    if (!devices.length) {
      return this.p.notification.update({
        where: { id: notification.id },
        data: { status: 'SKIPPED', error: 'No enabled push device' },
      });
    }
    const messages = devices.map(d => ({
      to: d.expoPushToken,
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        const payload = JSON.parse(text) as {
          data?:
            | Array<{ status?: string; message?: string; details?: { error?: string } }>
    ...
    return this.p.notification.update({
      where: { id: notification.id },
      data: { status: 'FAILED', error: lastError.slice(0, 1000) },
    });
  }
}
